(async () => {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`GET /nodes failed: ${res.status} ${res.statusText}`);
    ...
    for (const node of nodes) {
      const answers = ANSWERS_BY_TITLE[node.title];
      if (answers) {
        result[node.id] = answers;
        matched.push(node.title);
      } else {
        unmatched.push(node.title);
      }
    }

    localStorage.setItem('reflection_answers', JSON.stringify(result));

    console.log(`%creflection_answers restored`, 'color: #4ade80; font-weight: bold');
    console.log(`Matched (${matched.length}):`, matched);
    if (unmatched.length) console.warn(`Unmatched nodes (${unmatched.length}):`, unmatched);

    const titlesWithAnswers = Object.keys(ANSWERS_BY_TITLE);
    const notFound = titlesWithAnswers.filter(t => !matched.includes(t));
    if (notFound.length) console.warn(`Hardcoded titles not found in /nodes (${notFound.length}):`, notFound);
  } catch (err) {
    console.error('Failed to restore reflection_answers:', err);
  }
})();
